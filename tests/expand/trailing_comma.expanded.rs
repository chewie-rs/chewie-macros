...
pub struct TrailingCommaError {
    source: Box<dyn std::error::Error>,
}
#[automatically_derived]
#[allow(unexpected_cfgs)]
impl ::core::fmt::Debug for TrailingCommaError {
    #[inline]
...
impl std::fmt::Display for TrailingCommaError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("{0}: {1}", "error with trailing comma", self.source))
    }
}
impl std::error::Error for TrailingCommaError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        Some(self.source.as_ref())
    }
}
#[allow(unexpected_cfgs)]
impl TrailingCommaError {
    ...
    pub fn downcast<E: std::error::Error + 'static>(
        ...
        self.source
            .downcast::<E>()
            .map(|boxed| *boxed)
            .map_err(|source| TrailingCommaError { source })
    }
